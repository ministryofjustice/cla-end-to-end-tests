...
def step_impl(context):
    ...
    assert callback_btn is not None, f"Could not find callback button for case {context.selected_case_ref}"
...
def step_impl(context):
    ...
    case_logs = context.helperfunc.get_case_callback_details_from_backend(context.selected_case_ref)
...
def step_impl(context):
    ...
    for callback in callbacks:
        assert callback["reference"] != context.selected_case_ref, f"Case {context.selected_case_ref} is still present in the callbacks dashboard"
